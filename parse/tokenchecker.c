...
/*   Updated: 2024/07/14 23:20:21 by hyungcho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	puterr_int_retuen(char *msg)
{
	puterr(msg);
	return (FAILURE);
}
	...
	while (token_list)
	{
		...
		if (token->type == T_PIPE)
		{
			if (prv_token == NULL || prv_token->type != T_WORD)
				return (puterr_int_retuen("wrong pipeline input"));
		}
		else if (token->type == T_REDIRECT)
		{
			if (token_list->next == NULL
				|| ((t_token *)token_list->next->content)->type != T_WORD)
				return (puterr_int_retuen("wrong redirect input"));
		}
